...
export const createWorkshop = handleRequest(async (req, res) => {
  ...
  if (overlappingWorkshop) {
    res.status(400).json({
      message:
        "A workshop in the same category is already scheduled at this time.",
    });
    return;
  }
...
export const updateWorkshopPost = handleRequest(async (req, res) => {
  ...
  const post = await CommunityPost.findByIdAndUpdate(
    postId,
    { title: "🎉 Workshop is Live! Join Now", isPinned: true },
    { new: true }
  );
...
export const workshopCompletion = handleRequest(async (req, res) => {
  ...
  const communityPost = await CommunityPost.findOneAndUpdate(
    { workshopId: workshop._id },
    { isPinned: false }
  );
