...
const handleLike = async (postId: string, userId2: string) => {
  ...
  const Dropdown: React.FC<{ postId: string; userId: string }> = ({ postId, userId }) => {
    return (
      <motion.div
...
const CommentsModal: React.FC = () => {
    ...
    >
      ...
      >
        ...
        <div className="flex justify-between items-center mb-4 border-b border-gray-700 pb-3">
          <h2 className="text-white text-2xl font-semibold tracking-wide">Comments</h2>
          <motion.button
  ...
  const handleVote = async (postId: string, option: any, userId: string) => {
    ...
    const updatedOption = hasVoted ? option.votes.length - 1 : option.votes.length + 1;
              ...
              {posts.map((post: any, index: any) => (
                ...
                >
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex gap-4 cursor-pointer" onClick={() => handleProfileClick(post.user_id)}>
                      <motion.div
                      ...
                      >
                        ...
                        post.profileImage.includes("https://lh3.googleusercontent.com") ? (
                          ...
                          <CldImage src={post.profileImage} alt="Profile Image" width={80} height={80} className="w-full h-full object-cover" />
                        )
                      ) : (
                        <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRm59k-5YeirfW5MOf8SJiGIEJ6yTYRlnCs7SV93Y2__6FrKPWnE3FXgGDWhXAjsCe8_18&usqp=CAU" alt={`${post.newUsername || "User"} profile picture`} className="w-full h-full object-cover" />
                      )}
                      </motion.div>
                    ...
                    <motion.div whileHover={{ scale: 1.1 }} className="relative">
                      <button aria-label="More options" onClick={() => setActiveDropdown(activeDropdown === post._id ? null : post._id)}>
                  ...
                  {/* Post Content */}
                  <p className="text-white mt-4 text-[15px] leading-relaxed">{post.content}</p>
  
                  {/* Post Media */}
                  {post.image && (
                    <motion.div className="mt-4" >
                      <img src={post.image} alt="Post media" className="w-full aspect-video object-cover rounded-xl transition-all duration-300" />
                    </motion.div>
                  )}
      ...
      {post.poll &&
        post.poll.map((poll, index) => {
          ...
          return (
            ...
            >
              {/* Poll Question */}
              <h3 className="text-white font-extrabold text-lg text-center">{poll.question}</h3>
                ...
                {poll.options.map((option, optionIndex) => {
                  ...
                  return (
                    <motion.li
                      ...
                      transition={{ delay: 0.1 + optionIndex * 0.1, duration: 0.1 }}
                    ...
                    >
                      ...
                      <div className="flex flex-col">
                        <span className="text-white font-medium text-sm">{option.optionValue}</span>
                        <span className="text-gray-300 text-xs">{option.votes.length} votes</span>
                      </div>
                      ...
                      <motion.button
                        ...
                        onClick={() => handleVote(post._id, option, session?.user?.id)}
                      >
                        Vote
                      </motion.button>
                    </motion.li>
                  ...
                  <div className="flex gap-6 mt-4">
                    ...
                    >
                      <Heart className={`w-5 h-5 ${post.likes.some((like) => like.userId === session?.user?.id) ? "fill-current" : ""}`} />
